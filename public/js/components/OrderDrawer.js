...
function OrderDrawer({ order, isOpen, onClose, data, currentUser, handlers, onOpenPrintPreview }) {
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden select-none animate-fade-in flex items-end md:items-stretch justify-center md:justify-end">
      {/* Backdrop */}
      <div
        onClick={onClose}
      ...
      <div className="relative w-full md:w-screen md:max-w-2xl max-h-[92vh] md:max-h-full bg-white/95 dark:bg-slate-900/95 backdrop-blur-2xl border-t md:border-t-0 md:border-l border-white/80 dark:border-white/10 rounded-t-3xl md:rounded-none shadow-2xl flex flex-col liquid-specular pb-[max(1rem,env(safe-area-inset-bottom))] md:pb-0 z-10">
        ...
        {/* Header */}
        <div className="p-4 sm:p-5 border-b border-slate-200/80 dark:border-slate-800 bg-slate-50/70 dark:bg-slate-800/60 flex items-start justify-between gap-3">
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-1.5 sm:gap-2 mb-1 flex-wrap">
              <span className="font-mono font-black text-xs sm:text-sm bg-blue-100 dark:bg-blue-950 text-blue-700 dark:text-blue-300 px-2.5 py-0.5 rounded-lg border border-blue-300 dark:border-blue-800">
                {order.code}
              </span>
        ...
        <div className="px-4 sm:px-5 py-2.5 bg-slate-100/60 dark:bg-slate-800/40 border-b border-slate-200/80 dark:border-slate-800">
          <div className="flex items-center justify-between text-[11px] mb-1.5">
            <span className="font-bold text-slate-600 dark:text-slate-300">Tiến trình vòng đời:</span>
            <span className="font-mono font-bold text-blue-600 dark:text-blue-400">Bước {currentStatus.step}/9 ({currentStatus.label})</span>
          </div>
          <div className="w-full bg-slate-200 dark:bg-slate-700 h-2 rounded-full overflow-hidden flex">
            {Array.from({ length: 9 }).map((_, i) => (
              <div
                key={i}
                className={`flex-1 h-full border-r border-white/40 dark:border-slate-800 ${
        ...
        {/* Allocation Bar */}
        <div className="px-4 sm:px-5 py-2 bg-blue-50/50 dark:bg-blue-950/20 border-b border-blue-100 dark:border-blue-900/40 flex items-center justify-between gap-3 flex-wrap">
          <div className="flex items-center gap-2">
            <i className="fa-solid fa-boxes-stacked text-blue-600 dark:text-blue-400 text-xs"></i>
          ...
          <div className="flex items-center gap-2.5">
            <div className="w-24 sm:w-32 bg-slate-200 dark:bg-slate-700 h-2 rounded-full overflow-hidden">
              <div
                className={`h-full transition-all ${allocPercent === 100 ? 'bg-emerald-500' : 'bg-blue-600'}`}
        ...
        {/* Internal Tab Bar */}
        <div className="flex border-b border-slate-200/80 dark:border-slate-800 px-4 sm:px-5 gap-3 sm:gap-4 text-xs font-bold overflow-x-auto no-scrollbar">
          <button
            onClick={() => setActiveTab('bom')}
            className={`py-2.5 border-b-2 flex items-center gap-1.5 transition whitespace-nowrap ${
              activeTab === 'bom'
          ...
          >
            <i className="fa-solid fa-dolly"></i> Phiếu Xuất GDN ({relatedGdns.length})
          </button>
          <button
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-5 space-y-3 sm:space-y-4">
          ...
          {activeTab === 'bom' && (
            <div className="space-y-3">
              {!latestBom ? (
                <div className="text-center py-10 border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-2xl p-5">
                  ...
                  <button
                    ...
                    className="mt-3 px-4 py-2 bg-blue-600 hover:bg-blue-500 text-white font-bold text-xs rounded-xl shadow liquid-touch"
                  >
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between bg-slate-50 dark:bg-slate-800/50 p-3 rounded-2xl border border-slate-200/80 dark:border-slate-700">
                    <div>
                      <span className="text-xs font-bold text-slate-800 dark:text-slate-100">BOM Version {latestBom.version}</span>
                      <span className="ml-2 text-[10px] font-mono bg-blue-100 dark:bg-blue-950 text-blue-700 dark:text-blue-300 px-2 py-0.5 rounded">
                        {latestBom.status}
                      </span>
                    </div>
                  ...
                  <div className="md:hidden space-y-2">
                    {latestBom.items.map(item => {
                      ...
                      return (
                        <div key={item.id} className="p-3 bg-slate-50/90 dark:bg-slate-800/60 rounded-2xl border border-slate-200/80 dark:border-slate-700/60 space-y-1.5">
                          <div className="flex items-center justify-between">
                            <span className="font-mono font-bold text-xs text-blue-600 dark:text-blue-400">{sku?.code || 'SKU'}</span>
                            {isFulfilled ? (
                          ...
                          <div className="grid grid-cols-3 gap-2 pt-1 border-t border-slate-200/60 dark:border-slate-700/40 text-center font-mono text-[11px]">
                            <div className="bg-white dark:bg-slate-900/60 p-1 rounded-lg">
                            ...
                            <div className="bg-indigo-50 dark:bg-indigo-950/40 p-1 rounded-lg">
                            ...
                            <div className="bg-slate-100 dark:bg-slate-800 p-1 rounded-lg">
                              ...
                              <div className={`font-bold ${shortage > 0 ? 'text-red-600 dark:text-red-400' : 'text-emerald-600 dark:text-emerald-400'}`}>
          ...
          {activeTab === 'gdn' && (
            <div className="space-y-3">
              {relatedGdns.length === 0 ? (
                ...
                relatedGdns.map(gdn => (
                  <div key={gdn.id} className="p-3.5 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-200/80 dark:border-slate-700 flex items-center justify-between gap-2">
                    <div>
                      <div className="flex items-center gap-2">
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        className="px-2.5 py-1 bg-white dark:bg-slate-900 hover:bg-slate-100 text-slate-700 dark:text-slate-200 rounded-xl border border-slate-300 dark:border-slate-700 text-xs font-semibold flex items-center gap-1.5 shadow-xs liquid-touch"
                      >
                        <i className="fa-solid fa-print text-blue-500"></i> In PXK
                      </button>
                    </div>
                  </div>
                ))
          ...
          {activeTab === 'schedule' && (
            <div className="space-y-4">
              <div className="space-y-2">
                <h4 className="text-xs font-bold text-slate-700 dark:text-slate-300 uppercase">Đăng ký ca lấy vật tư (Mẫu 1)</h4>
                {relatedPickups.length === 0 ? (
                  <p className="text-xs text-slate-400 py-2">Chưa có lượt đăng ký ca lấy nào.</p>
                ) : (
                  relatedPickups.map(pk => (
                    <div key={pk.id} className="p-3 bg-emerald-50/60 dark:bg-emerald-950/20 rounded-2xl border border-emerald-200 dark:border-emerald-800/40 text-xs flex justify-between items-center">
                      <div>
              ...
              <div className="space-y-2 pt-2 border-t border-slate-200 dark:border-slate-800">
                <h4 className="text-xs font-bold text-slate-700 dark:text-slate-300 uppercase">Đơn Mua Hàng Bù Thiếu (PO)</h4>
                {relatedPos.length === 0 ? (
                  <p className="text-xs text-slate-400 py-2">Không có PO mua bù nào phát sinh.</p>
                ) : (
                  relatedPos.map(po => (
                    <div key={po.id} className="p-3 bg-red-50/60 dark:bg-red-950/20 rounded-2xl border border-red-200 dark:border-red-800/40 text-xs flex justify-between items-center">
                      <div>
                        <div className="font-mono font-bold text-red-900 dark:text-red-300">{po.code} - {po.supplierName}</div>
                        <div className="text-[11px] text-slate-500">Giá trị: {formatMoney(po.totalAmount)} | Dự kiến về: {formatDate(po.expectedDate)}</div>
                      </div>
                      <span className="font-mono text-[10px] bg-red-200 dark:bg-red-900 px-2 py-0.5 rounded text-red-800 dark:text-red-200 font-bold">
                        {po.status}
                      </span>
                    </div>
                  ))
          ...
          <div className="flex items-center gap-2">
            {order.status === 'CHO_BOM' && (
              <button
                ...
                className="px-3.5 py-2 bg-blue-600 hover:bg-blue-500 text-white font-bold text-xs rounded-xl shadow liquid-touch"
              >
            ...
            {latestBom && latestBom.status === 'SUBMITTED' && (
              <button
                ...
                className="px-3.5 py-2 bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-xs rounded-xl shadow liquid-touch"
              >
            ...
            {order.status === 'SAN_SANG_XUAT' && (
              <button
                ...
                className="px-3.5 py-2 bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs rounded-xl shadow liquid-touch"
              >
